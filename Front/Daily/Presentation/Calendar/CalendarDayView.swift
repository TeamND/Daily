...
struct DailyWeeklySummary: View {
    ...
    private var ratingsForChart: [RatingOnWeekModel] {
        ...
        var ratingsForChart: [RatingOnWeekModel] = Array(repeating: RatingOnWeekModel(day: "", rating: 0.0), count: 7)
        
        for index in 0 ..< ratingsForChart.count {
            ratingsForChart[index].day = DayOfWeek.text(for: index) ?? ""
        }
        
        for record in records {
            if let dayIndex = calendar.dateComponents([.weekday], from: record.date).weekday {
                let index = dayIndex - 1
                if record.isSuccess { ratingsForChart[index].rating += 1 }
            }
        }
        ...
        for (index, totalRecords) in recordsByDay {
            ratingsForChart[index].rating = ratingsForChart[index].rating / Double(totalRecords.count) * 100
        }
                ...
                VStack(alignment: .center, spacing: CGFloat.fontSize * 3) {
                    ...
                    Chart {
                        ForEach(ratingsForChart) { data in
                            ...
                            if ratingOfWeek > 0 {
                                RuleMark(
                                    y: .value("RatingOfWeek", ratingOfWeek)
                                )
